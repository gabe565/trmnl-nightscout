...
func (f *Fetch) fetchEntries(ctx context.Context) ([]nightscout.SGVv1, error) {
	...
	q := u.Query()
	start := time.Now().Add(-f.config.GraphDuration)
	q.Set("find[date][$gte]", strconv.FormatInt(start.Unix(), 10))
	q.Set("count", strconv.FormatInt(int64(f.config.GraphDuration.Minutes()/5), 10))
	u.RawQuery = q.Encode()
