...
func TestNewDots(t *testing.T) {
	...
	tests := []struct {
		...
		{"3 5", args{3, 5, false}, &DotImage{4, 6, 0, Palette1Bit()}},
		{"3 5 stagger", args{3, 5, true}, &DotImage{4, 6, 2, Palette1Bit()}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dots := NewDots(tt.args.x, tt.args.y, tt.args.stagger)
...
func TestDotImage_Stagger(t *testing.T) {
	d := NewDots(3, 3, true) // spaceX=4, spaceY=4, XOffset=2
